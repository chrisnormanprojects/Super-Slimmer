...
const starterFoods=[
 f('fresh-apple','Apple','Loose & fresh','Generic',52,'100g','🍎','fruit',free()),
 f('fresh-banana','Banana','Loose & fresh','Generic',89,'100g','🍌','fruit',free()),
 f('fresh-strawberry','Strawberries','Loose & fresh','Generic',32,'100g','🍓','fruit',free()),
 f('fresh-potato','Potatoes, plain','Loose & fresh','Generic',77,'100g','🥔','staple',free()),
 f('fresh-carrot','Carrots, plain','Loose & fresh','Generic',41,'100g','🥕','vegetable',free()),
 f('fresh-broccoli','Broccoli, plain','Loose & fresh','Generic',34,'100g','🥦','vegetable',free()),
 f('fresh-egg','Egg, whole','Loose & fresh','Generic',143,'100g','🥚','staple',free()),
 f('fresh-chicken','Chicken breast, skinless','Loose & fresh','Generic',120,'100g','🍗','lean-protein',free()),
 f('fresh-cod','Cod, plain','Loose & fresh','Generic',82,'100g','🐟','lean-protein',free()),
 f('dry-pasta','Dried pasta, plain','Packets & cupboard','Generic',350,'100g dry','🍝','staple',free(SOURCE_SHOP)),
 f('dry-rice','Long grain rice, plain','Packets & cupboard','Generic',360,'100g dry','🍚','staple',free(SOURCE_SHOP)),
 f('tin-tomato','Chopped tomatoes, canned, plain','Tinned & jars','Generic',24,'100g','🥫','vegetable',free(SOURCE_SHOP)),
 f('tin-beans','Baked beans in tomato sauce','Tinned & jars','Generic',78,'100g','🥫','plant-protein',free(SOURCE_SHOP)),
 f('tin-tuna','Tuna in spring water, drained','Tinned & jars','Generic',116,'100g','🥫','lean-protein',free(SOURCE_SHOP)),
 f('chilled-yogurt','Fat-free natural yogurt','Chilled','Generic',56,'100g','🥣','dairy',free()),
...
async function searchFoods(q){
 ...
 try{
  ...
  if(/^\d{8,14}$/.test(q)){
   ...
   const r=await fetch(url),j=await r.json();if(j.status===1&&j.product)products=[normalizeProduct(j.product)];
  }else{
   ...
   let r=await fetch(`https://world.openfoodfacts.org/cgi/search.pl?${params}`),j=await r.json();products=(j.products||[]).map(normalizeProduct).filter(p=>p.name!=='Unnamed product');
...
function openEditor(p){
 ...
 document.querySelector('#confirmedStatus').value=s.status||inferred.status||'unknown';
 document.querySelector('#confirmedValue').value=s.value??(inferred.status==='swips'&&Number.isFinite(inferred.value)?inferred.value:'');
 document.querySelector('#confirmedPortion').value=s.portion||p.portion||'';document.querySelector('#confirmedNote').value=s.note||'';syncEditor();
 document.querySelector('#deleteSavedBtn').classList.toggle('hidden',!stored()[p.id]);editDialog.showModal();
}
function syncEditor(){document.querySelector('#swipValueLabel').classList.toggle('hidden',document.querySelector('#confirmedStatus').value!=='swips')}
document.querySelector('#confirmedStatus').onchange=syncEditor;
document.querySelector('#saveConfirmedBtn').onclick=()=>{
 const status=document.querySelector('#confirmedStatus').value,raw=document.querySelector('#confirmedValue').value;if(status==='swips'&&raw===''){toast('Enter the confirmed Swip value');return}
 const s=stored();s[editing.id]={status,value:status==='swips'?Number(raw):null,portion:document.querySelector('#confirmedPortion').value.trim(),note:document.querySelector('#confirmedNote').value.trim(),savedAt:new Date().toISOString(),product:editing};saveStored(s);editDialog.close();
...
async function startScanner(){const dialog=document.querySelector('#scanDialog'),status=document.querySelector('#scanStatus'),video=document.querySelector('#scanVideo');dialog.showModal();if(!('BarcodeDetector'in window)){status.textContent='This browser does not expose direct barcode detection. Type the barcode into Search instead.';return}try{scanStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}}});video.srcObject=scanStream;const det=new BarcodeDetector({formats:['ean_13','ean_8','upc_a','upc_e']});status.textContent='Point the camera at the product barcode.';const tick=async()=>{if(!dialog.open)return;try{const codes=await det.detect(video);if(codes[0]?.rawValue){document.querySelector('#searchInput').value=codes[0].rawValue;stopScanner();dialog.close();searchFoods(codes[0].rawValue);return}}catch{}scanTimer=requestAnimationFrame(tick)};tick()}catch{status.textContent='Camera access was unavailable. You can type the barcode into Search.'}}
